...
CONFIG_PKG=github.com/fahimahamedwork/openagent/pkg/config
...
OPENAGENT_HOME?=$(HOME)/.openagent
WORKSPACE_DIR?=$(OPENAGENT_HOME)/workspace
...
## build: Build the openagent binary for current platform
build: generate
...
## build-launcher: Build the openagent-launcher (web console) binary
build-launcher:
        @echo "Building openagent-launcher for $(PLATFORM)/$(ARCH)..."
        @mkdir -p $(BUILD_DIR)
        @GOARCH=${ARCH} $(MAKE) -C web build \
                OUTPUT="$(CURDIR)/$(BUILD_DIR)/openagent-launcher-$(PLATFORM)-$(ARCH)$(EXT)" \
                WEB_GO='$(WEB_GO)' \
                GO_BUILD_TAGS='$(GO_BUILD_TAGS)' \
                LDFLAGS='$(LDFLAGS)'
        @$(LNCMD) openagent-launcher-$(PLATFORM)-$(ARCH)$(EXT) $(BUILD_DIR)/openagent-launcher$(EXT)
        @echo "Build complete: $(BUILD_DIR)/openagent-launcher$(EXT)"

build-launcher-frontend:
        @$(MAKE) -C web build-frontend

## build-launcher-tui: Build the openagent-launcher TUI binary
build-launcher-tui:
        @echo "Building openagent-launcher-tui for $(PLATFORM)/$(ARCH)..."
        @mkdir -p $(BUILD_DIR)
        @$(GO) build $(GOFLAGS) -o $(BUILD_DIR)/openagent-launcher-tui-$(PLATFORM)-$(ARCH) ./cmd/openagent-launcher-tui
        @ln -sf openagent-launcher-tui-$(PLATFORM)-$(ARCH) $(BUILD_DIR)/openagent-launcher-tui
        @echo "Build complete: $(BUILD_DIR)/openagent-launcher-tui"
...
## build-all: Build openagent for all platforms
build-all: generate
        @echo "Building for multiple platforms..."
        @mkdir -p $(BUILD_DIR)
...
## install: Install openagent to system and copy builtin skills
install: build
...
## uninstall: Remove openagent from system
uninstall:
...
## uninstall-all: Remove openagent and all data
uninstall-all:
        @echo "Removing workspace and skills..."
        @rm -rf $(OPENAGENT_HOME)
        @echo "Removed workspace: $(OPENAGENT_HOME)"
...
vet: generate
        @packages="$$($(GO) list $(GOFLAGS) ./...)" && \
                $(GO) vet $(GOFLAGS) $$(printf '%s\n' "$$packages" | grep -v '^github.com/fahimahamedwork/openagent/web/')
        @cd web/backend && $(WEB_GO) vet ./...

## test: Test Go code
test: generate
        @$(GO) test $(GOFLAGS) $$($(GO) list $(GOFLAGS) ./... | grep -v github.com/fahimahamedwork/openagent/web/)
        @cd web && make test
...
docker-build:
        ...
        docker compose -f docker/docker-compose.yml build openagent-agent openagent-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
        @echo "Building full-featured Docker image (Node.js 24)..."
        docker compose -f docker/docker-compose.full.yml build openagent-agent openagent-gateway

## docker-run: Run openagent gateway in Docker (Alpine-based)
docker-run:
        docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run openagent gateway in Docker (full-featured)
docker-run-full:
        docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run openagent agent in Docker (interactive, Alpine-based)
docker-run-agent:
        docker compose -f docker/docker-compose.yml run --rm openagent-agent

## docker-run-agent-full: Run openagent agent in Docker (interactive, full-featured)
docker-run-agent-full:
        docker compose -f docker/docker-compose.full.yml run --rm openagent-agent

## docker-clean: Clean Docker images and volumes
docker-clean:
        docker compose -f docker/docker-compose.yml down -v
        docker compose -f docker/docker-compose.full.yml down -v
        docker rmi openagent:latest openagent:full 2>/dev/null || true


## build-macos-app: Build OpenAgent macOS .app bundle (no terminal window)
build-macos-app:build-launcher
        ...
        @echo "macOS .app bundle created: $(BUILD_DIR)/OpenAgent.app"

## help: Show this help message
help:
        @echo "openagent Makefile"
        @echo ""
        @echo "Usage:"
        @echo "  make [target]"
        @echo ""
        ...
        @echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.openagent/workspace)"
