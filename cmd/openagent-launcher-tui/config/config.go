...
// Package config provides types and I/O for ~/.openagent/tui.toml.
package config
...
func DefaultConfigPath() string {
	...
	return filepath.Join(home, ".openagent", "tui.toml")
}

// TUIConfig is the top-level structure of ~/.openagent/tui.toml.
...
// SyncSelectedModelToMainConfig syncs the currently selected model to ~/.openagent/config.json
...
func SyncSelectedModelToMainConfig(scheme Scheme, user User, modelID string) error {
	...
	mainConfigPath := filepath.Join(home, ".openagent", "config.json")
