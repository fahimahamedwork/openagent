...
func startGateway() error {
	...
	if runtime.GOOS == "windows" {
		cmd = exec.Command("cmd", "/C", "start /B openagent gateway > NUL 2>&1")
	} else {
		cmd = exec.Command("sh", "-c", "nohup openagent gateway > /dev/null 2>&1 &")
	}
	...
	if runtime.GOOS == "windows" {
		cmd := exec.Command(
			...
			"process",
			"where",
			"name='openagent.exe' and commandline like '%gateway%'",
			"get",
			"processid",
		)
