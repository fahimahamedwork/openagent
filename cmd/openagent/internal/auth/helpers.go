...
import (
	...
	"github.com/fahimahamedwork/openagent/pkg/auth"
...
func authLoginGoogleAntigravity() error {
	...
	fmt.Println("Try it: openagent agent -m \"Hello world\"")
...
func authStatusCmd() error {
	...
	if len(store.Credentials) == 0 {
		...
		fmt.Println("Run: openagent auth login --provider <name>")
		return nil
	}
...
func authModelsCmd() error {
	...
	if err != nil || cred == nil {
		return fmt.Errorf(
			"not logged in to Google Antigravity.\nrun: openagent auth login --provider google-antigravity",
		)
	}
