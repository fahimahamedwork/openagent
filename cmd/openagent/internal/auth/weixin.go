...
import (
	...
	"github.com/fahimahamedwork/openagent/pkg/channels/weixin"
...
to authorize your account. On success, the bot token is saved to the openagent
config so you can start the gateway immediately.

Example:
  openagent auth weixin`,
...
func printManualWeixinConfig(token, baseURL string) {
	fmt.Println()
	fmt.Println("Add the following to the channels section of your openagent config:")
	fmt.Println()
	fmt.Println(`  "weixin": {`)
