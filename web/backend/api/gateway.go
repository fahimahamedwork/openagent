...
import (
	...
	ppid "github.com/fahimahamedwork/openagent/pkg/pid"
...
var gateway = struct {
	...
	logs                *LogBuffer
	pidData             *ppid.PidFileData // pid file data read from openagent.pid.json
...
// isLikelyGatewayProcess returns whether PID appears to be a openagent gateway
...
func isLikelyGatewayProcess(pid int) (bool, bool) {
	...
	if runtime.GOOS == "windows" {
		...
		if strings.HasPrefix(line, "\"") {
			if strings.Contains(line, "\"openagent.exe\"") {
				return true, true
			}
			return false, false
		}
...
// represents "openagent gateway ..." regardless of executable filename.
...
func (h *Handler) validateGatewayPidData(
	...
	if gatewayProcess, inspected := gatewayProcessMatcher(pidData.PID); inspected {
		if !gatewayProcess {
			return false, true, "pid process command is not openagent gateway"
		}
		return true, true, ""
	}
...
func (h *Handler) startGatewayLocked(initialStatus string, existingPid int) (int, error) {
	...
	// Start new process
	// Locate the openagent executable
	execPath := utils.FindOpenAgentBinary()
	...
	pid = cmd.Process.Pid
	logger.InfoC("gateway", fmt.Sprintf("Started openagent gateway (PID: %d) from %s", pid, execPath))
...
// handleGatewayStart starts the openagent gateway subprocess.
//
