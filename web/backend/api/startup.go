...
const (
	autoStartEntryName = "OpenAgentLauncher"
	launchAgentLabel   = "io.openagent.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "openagent-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=OpenAgent Web",
			"Comment=Start OpenAgent Web on login",
