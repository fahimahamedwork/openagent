...
var (
	...
	findOpenAgentBinaryForInfo   = resolveGatewayBinaryForVersionInfo
	runOpenAgentVersionOutput    = executeOpenAgentVersion
...
// resolveSystemVersionInfo prefers the actual openagent binary version output,
...
func (h *Handler) resolveSystemVersionInfoUncached(ctx context.Context) systemVersionResponse {
	...
	execPath := strings.TrimSpace(findOpenAgentBinaryForInfo())
	if execPath == "" {
		return fallback
	}
	...
	output, err := runOpenAgentVersionOutput(cmdCtx, execPath)
	if err != nil {
		return fallback
	}

	parsed, ok := parseOpenAgentVersionOutput(output)
	if !ok {
		return fallback
	}
...
func resolveGatewayBinaryForVersionInfo() string {
	...
	return utils.FindOpenAgentBinary()
}
...
// executeOpenAgentVersion runs the version subcommand against the
// discovered openagent executable.
func executeOpenAgentVersion(ctx context.Context, execPath string) (string, error) {
...
// parseOpenAgentVersionOutput extracts version/build/go fields from CLI output.
// It accepts banner/ANSI-decorated output and only requires the version line.
func parseOpenAgentVersionOutput(raw string) (systemVersionResponse, bool) {
