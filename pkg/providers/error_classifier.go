...
// Error patterns organized by FailoverReason, matching OpenAgent production (~40 patterns).
var (
...
func ClassifyError(err error, provider, model string) *FailoverError {
	...
	// Message pattern matching (priority order from OpenAgent).
...
// Priority order matters (from OpenAgent classifyFailoverReason).
