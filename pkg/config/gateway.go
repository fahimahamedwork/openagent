...
type GatewayConfig struct {
	Host      string `json:"host"                env:"OPENAGENT_GATEWAY_HOST"`
	Port      int    `json:"port"                env:"OPENAGENT_GATEWAY_PORT"`
	HotReload bool   `json:"hot_reload"          env:"OPENAGENT_GATEWAY_HOT_RELOAD"`
	LogLevel  string `json:"log_level,omitempty" env:"OPENAGENT_LOG_LEVEL"`
}
...
// The OPENAGENT_LOG_LEVEL environment variable overrides the file value.
func ResolveGatewayLogLevel(path string) string {
	...
	if envLevel := os.Getenv("OPENAGENT_LOG_LEVEL"); envLevel != "" {
