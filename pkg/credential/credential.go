...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via OPENAGENT_KEY_PASSPHRASE
...
//  2. OPENAGENT_SSH_KEY_PATH env var
//  3. ~/.ssh/openagent_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "OPENAGENT_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading OPENAGENT_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "OPENAGENT_SSH_KEY_PATH"

// openagentHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const openagentHome = "OPENAGENT_HOME"

const (
	FileScheme = "file://"
	EncScheme  = "enc://"

	hkdfInfo = "openagent-credential-v1"
	saltLen  = 16
	nonceLen = 12
	keyLen   = 32
)
...
//
// passphrase is required (OPENAGENT_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// OPENAGENT_SSH_KEY_PATH env var or ~/.ssh/openagent_ed25519.key.
...
//   - exact match with OPENAGENT_SSH_KEY_PATH env var
//   - within the OPENAGENT_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with OPENAGENT_SSH_KEY_PATH.
	...
	// Within OPENAGENT_HOME.
	if picoHome := os.Getenv(openagentHome); picoHome != "" {
...
// Final key: HKDF-SHA256(ikm, salt, info="openagent-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set OPENAGENT_SSH_KEY_PATH or place key at ~/.ssh/openagent_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (OPENAGENT_SSH_KEY_PATH, OPENAGENT_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. OPENAGENT_SSH_KEY_PATH env var
//  3. ~/.ssh/openagent_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the openagent-specific SSH key path if it exists.
