...
func DownloadAndExtractRelease(releaseURL, platform, arch string) (string, error) {
	...
	// Extract
	destDir, err := os.MkdirTemp("", "openagent-extract-*")
	if err != nil {
		os.Remove(tmpPath)
		return "", err
	}
...
// Example: owner="sky5454" -> https://api.github.com/repos/sky5454/openagent/releases/latest
func GetReleaseAPIURL(owner string) string {
	return fmt.Sprintf("https://api.github.com/repos/%s/openagent/releases/latest", owner)
}

// GetProdReleaseAPIURL returns the production release API URL (upstream).
func GetProdReleaseAPIURL() string {
	return GetReleaseAPIURL("fahimahamedwork")
}

// GetReleaseTagAPIURL returns the GitHub Releases API URL for a specific tag.
// Example: owner="fahimahamedwork", tag="nightly" -> https://api.github.com/repos/fahimahamedwork/openagent/releases/tags/nightly
func GetReleaseTagAPIURL(owner, tag string) string {
	return fmt.Sprintf("https://api.github.com/repos/%s/openagent/releases/tags/%s", owner, tag)
}

// GetNightlyReleaseAPIURL returns the nightly release API URL for the production repo.
func GetNightlyReleaseAPIURL() string {
	return GetReleaseTagAPIURL("fahimahamedwork", "nightly")
}
