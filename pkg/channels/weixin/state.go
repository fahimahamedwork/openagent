...
import (
	...
	basechannels "github.com/fahimahamedwork/openagent/pkg/channels"
...
func buildWeixinSyncBufPath(cfg config.WeixinConfig) string {
	return filepath.Join(openagentHomeDir(), "channels", "weixin", "sync", genWeixinAccountKey(cfg)+".json")
}

func buildWeixinContextTokensPath(cfg config.WeixinConfig) string {
	return filepath.Join(openagentHomeDir(), "channels", "weixin", "context-tokens", genWeixinAccountKey(cfg)+".json")
}
